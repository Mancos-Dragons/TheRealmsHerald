...
export default class RumorsModel {
        ...
        this.grammar = {
            es: {
                intros: [
                    ...
                    "Dicen las malas lenguas que", "Nadie confía en él, dicen que", "Hay rumores de que"
                ],
                subjects: [
                    "{npcName}, el {npcRole}", "{npcName} (el {npcRole})", "nuestro {npcRole}, {npcName}",
                    "el misterioso {npcName}, conocido {npcRole}", "alguien llamado {npcName}, que dice ser {npcRole}"
                ],
                actions: [
                    "fue visto haciendo tratos oscuros", "sabe dónde está el tesoro perdido", "está invocando fuerzas oscuras",
                    "hizo un pacto con un demonio de encrucijada", "es en realidad un espía del reino vecino",
                    "fue visto desenterrando algo", "está fabricando venenos para un asesino a sueldo"
                ],
                locations: [
                    "cerca del bosque viejo en {townName}", "en las afueras de {townName}", "en el viejo cementerio de {townName}",
                    "en el centro de {townName}", "en la taberna local de {townName}", "cerca de la plaza principal de {townName}"
                ],
                hooks: [
            ...
            en: {
                intros: [
                    "They say", "Some whisper that", "There were strange noises and they think",
                    "Rumor has it that", "Nobody trusts them, it's said that", "There are rumors that"
                ],
                subjects: [
                    "{npcName}, the {npcRole}", "{npcName} (the {npcRole})", "our {npcRole}, {npcName}",
                    "the mysterious {npcName}, known {npcRole}", "someone named {npcName}, claiming to be {npcRole}"
                ],
                actions: [
                    "was seen making dark deals", "knows where the lost treasure is", "is summoning dark forces",
                    "made a pact with a crossroad demon", "is actually a spy from the neighboring kingdom",
                    "was seen digging something up", "is crafting poisons for a hired assassin"
                ],
                locations: [
                    "near the old forest in {townName}", "on the outskirts of {townName}", "in the old graveyard of {townName}",
                    "in the center of {townName}", "in the local tavern of {townName}", "near the main square of {townName}"
                ],
                hooks: [
    ...
    async generateRumor(townName, npcName, npcRole) {
        ...
        const intro = getRandomElement(grammar.intros);
        const subject = getRandomElement(grammar.subjects);
        const action = getRandomElement(grammar.actions);
        const location = getRandomElement(grammar.locations);
        const hookTemplate = getRandomElement(grammar.hooks);

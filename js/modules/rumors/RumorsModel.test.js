...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor contains the required replaced variables
...
test('RumorsModel - uses defaults when arguments are empty', async (t) => {
    ...
    assert.ok(result.rumor.includes(expectedTown), 'Rumor should contain the default town name');
    assert.ok(result.rumor.includes(expectedNpc), 'Rumor should contain the default NPC name');
    assert.ok(result.rumor.includes(expectedRole), 'Rumor should contain the default NPC role');

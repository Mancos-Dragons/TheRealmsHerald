...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor has the required properties
    assert.ok(result.rumor.includes(townName), `Rumor should include townName: ${townName}`);
    assert.ok(result.rumor.includes(npcName), `Rumor should include npcName: ${npcName}`);
    assert.ok(result.rumor.includes(npcRole), `Rumor should include npcRole: ${npcRole}`);

    // Assert that the generated hook is one of the procedural plot hooks in grammar

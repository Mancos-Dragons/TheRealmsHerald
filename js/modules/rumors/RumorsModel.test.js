...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor correctly injects the variables
    ...
    assert.ok(model.grammar.es.plotHooks.includes(result.hook), 'Hook should match a procedural plot hook');

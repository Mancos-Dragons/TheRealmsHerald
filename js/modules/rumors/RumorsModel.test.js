...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Verify all variables are successfully replaced
    assert.ok(result.rumor.includes('Townsville') || result.rumor.includes('Bob') || result.rumor.includes('Baker'), 'Variables should be replaced in rumor');

    // Check that there are no unreplaced curly braces in the output (ignoring JSON structure braces if any, but since it's procedural text there shouldn't be {townName} etc left over)
    assert.strictEqual(result.rumor.includes('{townName}'), false, 'Should not contain {townName}');
    assert.strictEqual(result.rumor.includes('{npcName}'), false, 'Should not contain {npcName}');
    assert.strictEqual(result.rumor.includes('{npcRole}'), false, 'Should not contain {npcRole}');

    assert.strictEqual(result.hook.includes('{townName}'), false, 'Should not contain {townName}');
    assert.strictEqual(result.hook.includes('{npcName}'), false, 'Should not contain {npcName}');
    assert.strictEqual(result.hook.includes('{npcRole}'), false, 'Should not contain {npcRole}');

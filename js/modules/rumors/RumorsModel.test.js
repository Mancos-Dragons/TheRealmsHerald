...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert the presence of all required pre-loaded variables in the generated text
    ...
    assert.ok(result.hook.includes('Townsville'), 'Hook should contain the town name');
    assert.ok(result.hook.includes('Bob'), 'Hook should contain the npc name');

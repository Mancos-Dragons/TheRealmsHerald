...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor contains guaranteed items based on procedural grammar logic
    assert.ok(result.rumor.includes('Bob'));
    assert.ok(result.rumor.includes('Baker'));
    // Since {townName} is optional based on the locations grammar, we don't assert it strictly.

    assert.ok(result.hook.length > 10, 'Hook should have text');

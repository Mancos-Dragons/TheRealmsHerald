...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Verify it contains the required variables
    ...
    // Verify the hook is from the grammar
...
test('RumorsModel - uses defaults when arguments are empty', async (t) => {
    ...
    assert.ok(result.rumor.includes(expectedTown), 'Rumor should contain default town');
    assert.ok(result.rumor.includes(expectedNpc), 'Rumor should contain default NPC');
    assert.ok(result.rumor.includes(expectedRole), 'Rumor should contain default role');

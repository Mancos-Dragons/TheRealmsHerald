...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Call generateRumor
    const result = await model.generateRumor('Testville', 'Bob', 'Baker');

    // Assert that result has rumor and hook properties
    assert.ok(result.rumor);
    assert.ok(result.hook);

    // Assert that the generated rumor contains the interpolated variables
    assert.ok(result.rumor.includes('Testville'), 'Rumor should contain the town name');

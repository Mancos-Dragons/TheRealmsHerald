...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Validate that required variables are correctly replaced
    assert.ok(result.rumor.includes('Townsville') || result.rumor.includes('Bob') || result.rumor.includes('Baker'));

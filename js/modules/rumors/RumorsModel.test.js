...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor contains the injected variables
    assert.ok(result.rumor.includes(townName), `Rumor should contain the town name. Rumor: ${result.rumor}`);
    assert.ok(result.rumor.includes(npcName), `Rumor should contain the npc name. Rumor: ${result.rumor}`);
    assert.ok(result.rumor.includes(npcRole), `Rumor should contain the npc role. Rumor: ${result.rumor}`);
...
test('RumorsModel - uses defaults when arguments are empty', async (t) => {
    ...
    assert.ok(result.rumor.includes(expectedTown), `Rumor should contain the default town name. Rumor: ${result.rumor}`);
    assert.ok(result.rumor.includes(expectedNpc), `Rumor should contain the default npc name. Rumor: ${result.rumor}`);
    assert.ok(result.rumor.includes(expectedRole), `Rumor should contain the default npc role. Rumor: ${result.rumor}`);

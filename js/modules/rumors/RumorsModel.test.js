...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor contains the required placeholders which should have been replaced
    ...
    // Check that the generated hook is one of the procedurally generated ones
    const hooks = model.grammar.es.hooks;
    assert.ok(hooks.includes(result.hook), 'Hook should match a procedural plot hook');
...
test('RumorsModel - uses defaults when arguments are empty', async (t) => {
    ...
    assert.ok(result.rumor.includes(expectedTown) && result.rumor.includes(expectedNpc) && result.rumor.includes(expectedRole), "Rumor should contain the default values");

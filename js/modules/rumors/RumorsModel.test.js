...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Verify injected variables are present
    assert.ok(result.rumor.includes('Townsville'), 'Rumor should contain injected townName');
    assert.ok(result.rumor.includes('Bob'), 'Rumor should contain injected npcName');
    assert.ok(result.rumor.includes('Baker'), 'Rumor should contain injected npcRole');

    // Assert that the generated hook is from the model's grammar
    assert.ok(model.grammar.es.hooks.includes(result.hook), 'Hook should match a procedural plot hook from grammar');
...
test('RumorsModel - uses defaults when arguments are empty', async (t) => {
    ...
    assert.ok(
        ...
        'Rumor should contain default values when no args provided'
    );

...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor correctly incorporates variables
    assert.ok(result.rumor.includes(townName), 'Rumor should contain the town name');
    assert.ok(result.rumor.includes(npcName), 'Rumor should contain the NPC name');
    assert.ok(result.rumor.includes(npcRole), 'Rumor should contain the NPC role');

    // Assert that the generated hook is from the grammar

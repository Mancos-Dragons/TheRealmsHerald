...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor is based on the procedural grammar
    ...
    assert.strictEqual(foundIntro, true, 'Rumor should contain an intro fragment');

    let foundAction = false;
    for (const action of model.grammar.actions.es) {
        if (result.rumor.includes(action)) {
            foundAction = true;
            break;
        }
    }
    assert.strictEqual(foundAction, true, 'Rumor should contain an action fragment');

    let foundLocation = false;
    for (const location of model.grammar.locations.es) {
        if (result.rumor.includes(location)) {
            foundLocation = true;
            break;
        }
    }
    assert.strictEqual(foundLocation, true, 'Rumor should contain a location fragment');

    // All predefined variables should be present

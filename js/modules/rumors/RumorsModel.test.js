...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor strictly contains the parameters
    assert.ok(result.rumor.includes('Townsville'), 'Rumor text should contain the town name');
    assert.ok(result.rumor.includes('Bob'), 'Rumor text should contain the NPC name');
    assert.ok(result.rumor.includes('Baker'), 'Rumor text should contain the NPC role');

    // Assert that the generated hook strictly contains the parameters
    assert.ok(result.hook.includes('Townsville'), 'Hook text should contain the town name');
    assert.ok(result.hook.includes('Bob'), 'Hook text should contain the NPC name');
    assert.ok(result.hook.includes('Baker'), 'Hook text should contain the NPC role');

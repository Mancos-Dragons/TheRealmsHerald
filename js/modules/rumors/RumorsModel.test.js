...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    assert.strictEqual(result.rumor.includes('Townsville'), true, 'Rumor should include Townsville');
    assert.strictEqual(result.rumor.includes('Bob'), true, 'Rumor should include Bob');
    assert.strictEqual(result.rumor.includes('Baker'), true, 'Rumor should include Baker');

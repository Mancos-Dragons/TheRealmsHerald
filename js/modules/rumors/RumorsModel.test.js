...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor strictly includes all required variables
    assert.ok(result.rumor.includes('Townsville'), 'Rumor must include the town name');
    assert.ok(result.rumor.includes('Bob'), 'Rumor must include the NPC name');
    assert.ok(result.rumor.includes('Baker'), 'Rumor must include the NPC role');

...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor contains the required placeholders
    assert.ok(result.rumor.includes(townName), `Rumor should contain townName. Rumor: ${result.rumor}`);
    assert.ok(result.rumor.includes(npcName), `Rumor should contain npcName. Rumor: ${result.rumor}`);
    assert.ok(result.rumor.includes(npcRole), `Rumor should contain npcRole. Rumor: ${result.rumor}`);
...
test('RumorsModel - uses defaults when arguments are empty', async (t) => {
    ...
    assert.ok(result.rumor.includes(expectedTown) && result.rumor.includes(expectedNpc) && result.rumor.includes(expectedRole), "Rumor should contain the default placeholders");

...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor strictly incorporates the required pre-loaded variables
    assert.ok(result.rumor.includes('Townsville'), 'Rumor should include the townName');
    assert.ok(result.rumor.includes('Bob'), 'Rumor should include the npcName');
    assert.ok(result.rumor.includes('Baker'), 'Rumor should include the npcRole');

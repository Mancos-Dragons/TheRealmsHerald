...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor incorporates the npcName and npcRole
    assert.ok(result.rumor.includes('Bob'), 'Rumor should contain the npcName');
    assert.ok(result.rumor.includes('Baker'), 'Rumor should contain the npcRole');

    // Assert that the generated hook is one of the plot hooks from the model's grammar

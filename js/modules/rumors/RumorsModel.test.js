...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor contains the required variables
    assert.ok(result.rumor.includes('Townsville'), 'Rumor should contain town name');
    assert.ok(result.rumor.includes('Bob'), 'Rumor should contain NPC name');
    assert.ok(result.rumor.includes('Baker'), 'Rumor should contain NPC role');
...
test('RumorsModel - uses defaults when arguments are empty', async (t) => {
    ...
    assert.ok(result.rumor.includes(expectedTown), 'Rumor should contain default town name');
    assert.ok(result.rumor.includes(expectedNpc), 'Rumor should contain default NPC name');
    assert.ok(result.rumor.includes(expectedRole), 'Rumor should contain default NPC role');

...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor has substituted variables properly
    ...
    // Ensure it is structurally sound (doesn't have leftover braces)
    assert.ok(!result.rumor.includes('{townName}'));
    assert.ok(!result.rumor.includes('{npcName}'));
    assert.ok(!result.rumor.includes('{npcRole}'));

    // Assert that the hook is generated
    assert.ok(result.hook.length > 10, 'Hook should have some content');

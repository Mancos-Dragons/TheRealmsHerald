...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated hook contains the required variables
    assert.ok(result.hook.includes('Townsville'), 'Hook should include townName');
    assert.ok(result.hook.includes('Bob') || !result.hook.includes('{npcName}'), 'Hook should be formatted');
    assert.ok(result.hook.includes('Baker') || !result.hook.includes('{npcRole}'), 'Hook should be formatted');

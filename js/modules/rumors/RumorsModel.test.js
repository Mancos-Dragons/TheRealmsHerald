...
test('RumorsModel - fallback to procedural generation when AIService is not configured', async (t) => {
    ...
    // Assert that the generated rumor dynamically includes townName, npcName, and npcRole
    assert.ok(result.rumor.includes(townName), 'Rumor should include townName');
    ...
    // Assert that the generated hook is one of the grammar hooks
...
test('RumorsModel - uses defaults when arguments are empty', async (t) => {
    ...
    assert.ok(result.rumor.includes(expectedTown) && result.rumor.includes(expectedNpc) && result.rumor.includes(expectedRole), 'Rumor should include default values');
